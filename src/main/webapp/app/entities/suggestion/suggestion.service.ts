...
export class SuggestionService {
  ...
  protected convertDateFromClient(suggestion: ISuggestion): ISuggestion {
    const copy: ISuggestion = Object.assign({}, suggestion, {
      date: suggestion.date != null && suggestion.date.isValid() ? suggestion.date.format(DATE_FORMAT) : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((suggestion: ISuggestion) => {
        suggestion.date = suggestion.date != null ? moment(suggestion.date) : null;
      });
    }
    return res;
  }
